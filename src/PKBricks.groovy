...
def dataFolder = new File("/Users/edalquist/java/workspace_37/PKBricks/src/");
def historyFile = new File(dataFolder, "history.json");
def brickDefaultsFile = new File(dataFolder, "brickStatusDefaults.json");

def history;
if (historyFile.exists()) {
    history = jsonSlurper.parseText(historyFile.getText("UTF-8"));
}
else {
    history = [:];
}
...
if (brickDataFilename == null) {
    ...
    brickData = jsonSlurper.parseText(brickDefaultsFile.getText("UTF-8"));
    history[brickTimeStamp] = brickDataFilename;
}
else {
    brickDataFile = new File(dataFolder, brickDataFilename);
    brickData = jsonSlurper.parseText(brickDataFile.getText("UTF-8"));
}
...
brickDataFile.write(JsonOutput.prettyPrint(JsonOutput.toJson(brickData)), "UTF-8");
historyFile.write(JsonOutput.prettyPrint(JsonOutput.toJson(history)), "UTF-8");
